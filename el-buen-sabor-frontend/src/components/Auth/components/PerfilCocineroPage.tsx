// src/pages/cocinero/PerfilCocineroPage.tsx
...
export default function PerfilCocineroPage() {
  ...
  // currentDomicilio puede ser null para 'agregar' o un IDomicilioDTO para 'editar'
  ...
  const fetchPerfilData = async () => {
    ...
    try {
      setLoading(true);
      // El endpoint `/api/usuarios/perfil/${email}` es el general
  ...
  const handleAddOrEditDomicilio = () => {
    // Si perfil.domicilios[0] existe, se edita ese. Si no, se crea uno nuevo.
  ...
  const handleSaveDomicilio = async (domicilio: IDomicilioDTO) => {
    if (!userId) { // userId es el ID de la Persona (cocinero)
    ...
    try {
      ...
      if (domicilio.id && domicilio.id !== 0) {
        ...
        // Es una creación: asocia un nuevo domicilio a esta persona (cocinero)
      ...
      setPerfil(prev => ({
        ...(prev as PerfilGeneralDTO),
        domicilios: [savedDomicilio], // Ahora el cocinero tiene este único domicilio
      }));
  ...
  // Determinar si el cocinero tiene al menos un domicilio
    ...
    return (
        <div className="max-w-2xl mx-auto bg-white rounded-xl shadow p-8 mt-8 relative">
          ...
          <div className="flex items-center gap-4 mb-6">
            <div className="w-16 h-16 bg-gray-100 rounded-full flex items-center justify-center text-2xl text-green-600 font-bold">
              {perfil.nombre?.charAt(0)}
              {perfil.apellido?.charAt(0)}
            </div>
            <h2 className="text-2xl font-bold text-green-600">Mi Perfil de Cocinero 👨‍🍳</h2>
          </div>
  ...
  return (
      <div className="max-w-2xl mx-auto bg-white rounded-xl shadow p-8 mt-8">
        <h2 className="text-2xl font-bold mb-6 text-green-600">Editar Perfil</h2>
